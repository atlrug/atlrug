= atlrug Web Site (atlrug4)

This is a Ruby-on-Rails web application for the ATLRUG (Atlanta Ruby User Group).

{<img src="https://travis-ci.org/jasnow/altrug4.png" />}[https://travis-ci.org/jasnow/atlrug4.png]

{<img src="https://gemnasium.com/jasnow/atlrug4.png" />}[https://gemnasium.com/jasnow/atlrug4.png]

{<img src="https://codeclimate.com/github/jasnow/atlrug4.png" />}[https://codeclimate.com/github/jasnow/altrug4]

== Setup

1. To install gems, run: <tt>bundle install</tt>.

 * On 4/26/2013, the requirements were: Ruby 2.0.0-p0; Rails 4.0.0.rc1

2. Create databases and tables:

    rake db:create
    rake db:migrate

3. Run the tests by typing "rake". (when we write them - smile)

4. Project is on Github at https://github.com/jasnow/atlrug4.

5. Follow these instructions regarding Heroku and Rails 4.
 * https://gist.github.com/peter/3025502 (avoid "bundle --binstubs")

6. You have two env variable to set:
 * AR4_S_K_B (secret key base)
 * MEETUP_KEY (ENV var for meetup.com API key)
  * Used **travis encrypt MEETUP_KEY=${MEETUP_KEY} --add** to encrypt and add it to .travis.yml file.
  * You can get an meetup.com API key at **http://www.meetup.com/meetup_api/key** .
  * Make sure you do not have a $HOME/.travis/config file with your repos in it or it will lock down your key to only your repos.

7. For more detail about **Guard LiveReload*, go to https://github.com/guard/guard-livereload.

== To Contribute

1. Go over to the **Projects needing help:** page on http://contribhub.co web site and click the project's "Wanna Help!" button.

2. Fork project on Github.

3. Clone it locally.

4. Make Change.

5. Push it back to Github.

6. Create a Pull Request to original repo.

== DISCUSSION

If you have questions, please use this Google group: ** atlrug-website@googlegroups.com **

== License

atlrug4 is released under the MIT License. http://www.opensource.org/licenses/mit-license
